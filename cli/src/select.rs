...
fn exclude(prev: Option<WordId>) -> String {
    prev.map(|p| format!(" AND w.id != {}", p.0))
        .unwrap_or_default()
}
...
pub fn learning_word(
    ...
    let flag = |on: bool, s: &str| {
        ...
        let due = if any {
            ...
            format!(
                " AND w.t_{s} IS NOT NULL AND w.i_{s} IS NOT NULL AND w.t_{s} + w.i_{s} <= {now}"
            )
        };
        format!("(w.q_{s} = 1{due})")
    };
...
pub fn review_word(
    ...
    let sql = format!(
        ...
        review_flag(
            ...
            "rec",
            "rep",
            &format!(" AND w.t_rec + w.i_rec <= {now}{plus}")
        ),
        review_flag(
            rep_on,
            "rep",
            "rec",
            &format!(" AND w.t_rep + w.i_rep <= {now}{plus}")
        ),
        ...
        match (rec_on, rep_on) {
            (true, true) => format!(
                ...
                review_flag(
                    ...
                    "rec",
                    "rep",
                    &format!(" AND w.t_rec + w.i_rec <= {now} + 60")
                ),
                review_flag(
                    true,
                    "rep",
                    "rec",
                    &format!(" AND w.t_rep + w.i_rep <= {now} + 60")
                )
            ),
            (true, false) => review_flag(
                true,
                "rec",
                "rep",
                &format!(" AND w.t_rec + w.i_rec <= {now} + 60")
...
pub fn next(
    ...
    let news = learns
        && today
            .goal
            .is_none_or(|g| learning + today.learned_today < g);
    let reviews = session.reviews();
    let selected_only = rules.review_from == ReviewFrom::Selected;
    let new = |postponed: bool, rng: &mut Rng| {
        new_word(conn, scope, now, rules.new_words, postponed, rng)
    };
    ...
    if learns
        && learning > 0
        && let Some(p) = learn(true, prev, rng)?
    {
        return Ok(Some(p));
    }
    ...
    if learns
        && learning > 0
        && let Some(p) = learn(true, None, rng)?
    {
        return Ok(Some(p));
    }
...
pub fn variants(
    ...
    let cats: Vec<(String, bool, Option<String>)> = conn
        .prepare(&format!(
            ...
            name.as_deref()
                .map_or("NULL".to_string(), |n| format!("c.{n}"))
        ))?
        .query_map([word.id.0], |r| {
    ...
    let cat_named = name
        .map(|n| format!("        AND C.{n} IS NOT NULL"))
        .unwrap_or_default();
    let sql = format!(
    ...
    for id in ids {
        out.push(
            crate::store::get_word(conn, &id.to_string())?
                .with_context(|| format!("no word id {id}"))?,
        );
    }
    rng.shuffle(&mut out);
    Ok(out)
...
pub fn card_with(
    ...
    let word = crate::store::get_word(conn, &p.word.0.to_string())?
        .with_context(|| format!("no word id {}", p.word.0))?;
    let again: Option<Vec<Word>> = shown
        .iter()
        .map(|id| {
            crate::store::get_word(conn, &id.0.to_string())
                .ok()
                .flatten()
        })
        .collect();
...
mod tests {
    ...
    fn word(conn: &Connection, id: i64, text: &str, category: &str, set: &str) {
        ...
        .unwrap();
        if !set.is_empty() {
            conn.execute(&format!("UPDATE WORD SET {set} WHERE ID = ?"), [id])
                .unwrap();
        }
    }
    ...
    const DUE_REVIEW: &str =
        "Q_REC=2, Q_REP=2, T_REC=1000, T_REP=1000, I_REC=100, I_REP=100, S_REC=2, S_REP=2";
    ...
    fn scope_reads_native_language_and_region() {
        ...
        c.execute_batch("CREATE TABLE SETTINGS (NAME TEXT, VALUE TEXT);")
            .unwrap();
    ...
    fn new_words_draw_a_category_first() {
        ...
        c.execute("UPDATE WORD SET RUS = NULL WHERE ID = 32", [])
            .unwrap();
        word(
            &c,
            33,
            "postponed",
            "small",
            &format!("T_REC={NOW}, I_REC=100000"),
        );
        let s = scope(&c);
        let mut rng = Rng::new(7);
        let mut small = 0;
        for _ in 0..400 {
            let p = new_word(&c, &s, NOW, NewMode::Recognition, true, &mut rng)
                .unwrap()
                .unwrap();
            assert!(
                p.word.0 == 1 || (10..20).contains(&p.word.0),
                "picked {:?}",
                p.word
            );
    ...
    fn learning_words_wait_for_their_timer() {
        ...
        word(
            ...
            "a",
            &format!(
                "Q_REC=1, Q_REP=1, T_REC={}, T_REP={}, I_REC=30, I_REP=30",
                NOW - 100,
                NOW - 100
            ),
        );
        word(
            ...
            "waiting",
            "a",
            &format!("Q_REC=1, Q_REP=1, T_REC={NOW}, T_REP={NOW}, I_REC=30, I_REP=30"),
        );
        let s = scope(&c);
        let mut rng = Rng::new(1);
        for _ in 0..50 {
            let p = learning_word(&c, &s, NOW, SideMode::Reproduction, false, None, &mut rng)
                .unwrap()
                .unwrap();
    ...
    fn review_takes_the_ten_longest_due() {
        ...
        for id in 1..=12 {
            word(
                ...
                "a",
                &format!(
                    "Q_REC=2, Q_REP=2, T_REC={}, T_REP={}, I_REC=100, I_REP=100, S_REC=2, S_REP=2",
                    900 + id * 10,
                    900 + id * 10
                ),
            );
        ...
        word(
            ...
            "learning-other-side",
            "a",
            "Q_REC=2, Q_REP=1, T_REC=100, I_REC=100",
        );
        word(
            &c,
            21,
            "not-due",
            "a",
            &format!("Q_REC=2, Q_REP=2, T_REC={NOW}, T_REP={NOW}, I_REC=9999, I_REP=9999"),
        );
        let s = scope(&c);
        ...
        assert!(
            sides[0] > 100 && sides[1] > 100,
            "both sides drawn: {sides:?}"
        );
        let p = review_word(
            &c,
            &s,
            NOW,
            SideMode::Recognition,
            true,
            false,
            None,
            &mut rng,
        ...
        assert_eq!(
            due_count(&c, &s, NOW, SideMode::RecognitionOrReproduction, true).unwrap(),
            12
        );
        assert_eq!(
            next_review(&c, &s, SideMode::RecognitionOrReproduction, true).unwrap(),
            Some(950)
        );
    }
    ...
    fn review_grace_and_category_scope() {
        ...
        word(
            ...
            "soon",
            "on",
            &format!("Q_REC=2, Q_REP=2, T_REC={NOW}, T_REP={NOW}, I_REC=30, I_REP=30"),
        );
        ...
        assert!(
            review_word(&c, &s, NOW, rr, true, false, None, &mut rng)
                .unwrap()
                .is_none()
        );
        assert_eq!(
            review_word(&c, &s, NOW, rr, true, true, None, &mut rng)
                .unwrap()
                .unwrap()
                .word
                .0,
        ...
        assert_eq!(
            review_word(&c, &s, NOW, rr, false, false, None, &mut rng)
                .unwrap()
                .unwrap()
                .word
                .0,
        ...
        assert_eq!(
            due_count(&c, &s, NOW, rr, true).unwrap(),
            1,
            "the badge counts a minute ahead"
        );
    }
    ...
    fn goal_follows_the_day_row() {
        ...
        c.execute("INSERT INTO DAILY_GOAL VALUES ('2026-09-14', 30, 45)", [])
            .unwrap();
    ...
    fn sessions_draw_what_they_allow() {
        ...
        let d = today(&c, &rules);
        for _ in 0..50 {
            let p = next(&c, &rules, &s, Session::ReviewOnly, NOW, &d, None, &mut rng)
                .unwrap()
                .unwrap();
            assert_eq!(p.source, Source::Review);
            let p = next(&c, &rules, &s, Session::NewOnly, NOW, &d, None, &mut rng)
                .unwrap()
                .unwrap();
        ...
        for _ in 0..100 {
            let p = next(&c, &rules, &s, Session::Smart, NOW, &d, None, &mut rng)
                .unwrap()
                .unwrap();
    ...
    fn the_goal_caps_new_words() {
        ...
        word(
            ...
            "learning",
            "a",
            "Q_REC=1, Q_REP=1, T_REC=1, T_REP=1, I_REC=30, I_REP=30",
        );
        let rules = crate::store::rules(&c).unwrap();
        let s = scope(&c);
        ...
        assert!(
            next(&c, &rules, &s, Session::NewOnly, NOW, &d, None, &mut rng)
                .unwrap()
                .is_none()
        );
    }
    ...
    fn choose_ties_on_the_category_flag_like_the_phone() {
        ...
        assert!(
            others,
            "an equally ranked word from another selected category turns up"
        );
    }
    ...
    fn choose_offers_four_with_selected_categories_first() {
        ...
        for _ in 0..30 {
            ...
            assert_eq!(
                ids,
                vec![1, 2, 3, 4],
                "words from selected categories come first"
            );
        }
        assert!(
            variants(&c, &rules, &s, &w, Side::Rec, &mut rng)
                .unwrap()
                .is_empty()
        );
        let card = card(
    ...
    fn a_new_word_has_no_blocks() {
        let c = db(&[
            ...
            ("enable_words_keyboard_input", "both"),
        ]);
        cat(&c, "a", true);
        word(&c, 1, "fresh", "a", "");
        for id in 2..6 {
